<?php
 session_start();
  ...
  $info = "select * from customerbooking";
			...
			<table width="100%" style="background: #fff; border-radius: 5px;">
				...
				<tbody>
					...
					<tr>
						...
						<?php 
							$p_info = "select due from accounts where order_id = '$id'";
  							$getpinfo = $crud->getData($p_info);
  							foreach($getpinfo as $key=>$price_info) {
  							$due = $price_info['due'];
						...
						?>
						<td style="<?php echo $color?>"><strong>Tk <?php echo $due ?></strong></td>
						<?php }?>
						<td style="color: blue;"><a  href="orderdetails.php?idl=<?php echo $id?>"><strong>View</strong></a></td>	
